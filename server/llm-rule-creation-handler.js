/**
 * LLM Rule Creation Handler
 * Generates DRL rules using OpenAI API
 */

/**
 * Generate DRL rule using OpenAI
 * @param {string} userPrompt - User's request for rule generation
 ...
 */
export async function generateRuleWithLLM(userPrompt, documentContent, factObject, factSchema) {
  console.log(`[LLM] ========================================`)
  console.log(`[LLM] 🤖 RULE CREATION REQUEST`)
  console.log(`[LLM] ========================================`)
  ...
  const systemPrompt = `You are an expert Drools Rule Language (DRL) developer. Generate ONLY the new rule requested by the user.

IMPORTANT:
- Return ONLY the new rule (rule "name" ... end block)
- DO NOT include package, import, or existing rules
- DO NOT repeat the entire DRL file
- Generate only the single rule requested
...
Response format (JSON):
{
  ...
  const userMessage = buildUserMessage(userPrompt, documentContent, factContext)
  ...
  try {
    ...
    return {
      drl: result.drl || '',
      reasoning: result.reasoning || 'Rule generated successfully'
    }
  } catch (error) {
 ...
 */
function buildUserMessage(userPrompt, documentContent, factContext) {
  const existingCode = documentContent.trim() 
    ? `\n\nExisting DRL code (for context only - DO NOT repeat this):\n\`\`\`\n${documentContent}\n\`\`\``
...
IMPORTANT: Generate ONLY the new rule requested. Return JSON with:
- "drl": Only the new rule block (rule "name" ... end), no package/import
